...
/*
 * Fast Thread-wakeup Bad paste address test
 *	Test that when a thread writes to a BAD paste address, it gets a
 *	SIGSEGV, but that system is itself ok.
 *
 *	Create N_THREAD_PAIRS pairs of threads - first thread in each pair
 *	creates a receive window and issues Wait. Second thread in the pair,
 *	creates a send window but pastes to a _BAD_ address.
 */
...
static void *sender_func(void *arg)
{
	...
	printf("Writing to a bad paste address\n");
	rc = write_empty_crb((void *)65536, msg);
	if (rc < 0)
...
int main(int argc, char *argv[])
{
	...
	printf("ftw-1: Crated %d thread pairs\n", N_THREAD_PAIRS);
	/*
	...
	printf("ftw-1: %d thread pair joined\n", N_THREAD_PAIRS);
	_Exit(0);
}
