...
static atomic_t pending_faults = ATOMIC_INIT(0);
static atomic_t fault_crbs = ATOMIC_INIT(0);
...
static void update_csb(int pid, struct coprocessor_request_block *crb)
{
	...
	/*
	 * TODO: What format should we return the address in? CPU or BE?
	 * 	 i.e only one of us, either kernel (here) or user should
	 * 	 convert to CPU format. Not both!
	 */
	...
	/*
	 * TODO: Check PF_EXITING or other flags to ensure that the task
	 * 	 is not about to exit, specially for the use_mm() call
	 * 	 below.
	 */
	get_task_struct(tsk);
	rcu_read_unlock();
...
static void process_fault_crb(struct vas_instance *vinst)
{
	 ...
	 */
	fifo = vinst->fault_fifo + atomic_read(&fault_crbs) * CRB_SIZE;

	pr_err("VAS[%d] fault_fifo %p, fifo %p, faults %d pending %d\n",
			vinst->vas_id, vinst->fault_fifo, fifo,
			atomic_read(&fault_crbs), atomic_read(&pending_faults));

	memcpy(crb, fifo, CRB_SIZE);
	memset(fifo, 0, CRB_SIZE);

	/*
	 * TODO: Use a mutex instead of atomic here, since we have to
	 *       wrap the fault_crbs back to 0.
	 */
	atomic_inc(&fault_crbs);
	if (atomic_read(&fault_crbs) == vinst->fault_fifo_size/CRB_SIZE)
		atomic_set(&fault_crbs, 0);
